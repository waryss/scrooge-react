...
import {addExpense, logout, removeExpense, setFetchedExpenses, setVisibilityFilter, toggleExpense} from '../action';
import {deleteExpense, fetchExpenses, insertExpense, updateExpense} from '../hasuraApi';
...
class Expense extends Component {
    ...
    async componentDidMount() {
        let resp = await fetchExpenses(this.props.session.token);
        if (resp.status !== 200) {
            if (resp.status === 503) {
    ...
    async componentDidUpdate() {
        ...
        if (this.state.removedId != null) {
            ...
            let resp = await deleteExpense(expenseId, this.props.session.token);
            if (resp.status !== 200) {
                if (resp.status === 503) {
                    Alert.alert("Network Error", "Please check your internet connection");
                } else {
        ...
        if (this.state.toggledId != null) {
            ...
            let resp = await updateExpense(expenseId, !currentStatus, this.props.session.token);
            if (resp.status !== 200) {
                if (resp.status === 503) {
                    Alert.alert("Network Error", "Please check your internet connection");
                } else {
    ...
    onSubmit = async () => {
        ...
        if (this.state.inputText.length > 0) {
            let resp = await insertExpense(this.state.inputText, this.props.session.token);
            if (resp.status !== 200) {
                if (resp.status === 503) {
                    Alert.alert("Network Error", "Please check your internet connection");
                } else {
                ...
                let expenseId = respBody.returning[0].id;
